...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                method = getDeclaredMethod(clazz, route.getAction());
        ...
        Map<Object, Object> argumentsMap = new HashMap<>() {{
            put(Map.class, view);
        ...
        for (Class<?> parameterType : method.getParameterTypes()) {
            if (argumentsMap.containsKey(parameterType)) {
                arguments.add(argumentsMap.get(parameterType));
            }
        }

        try {
            method.invoke(page, arguments.toArray(new Object[arguments.size()]));
    ...
    private Method getDeclaredMethod(Class<?> clazz, String action) throws NoSuchMethodException {
